...
class Dataset(npfl138.TransformedDataset):
    ...
    def collate(self, batch):
        ...
        # TODO(lemmatizer_noattn): `words` is a list of lists of strings. Flatten it into a single list of strings
        ...
        lemmas = ...
        # TODO(lemmatizer_noattn): Return an (inputs, targets) pair, where
        # - the inputs are the words during inference and the (words, lemmas) pairs during training;
        # - the targets are the lemmas.
        return ..., ...
...
class WithAttention(torch.nn.Module):
    ...
    def setup_memory(self, encoded):
        ...
        # TODO: Pass `encoded` through `self._project_encoder_layer` and store
    ...
    def forward(self, inputs, states):
        ...
        # - Compute the projected decoder states by passing the given state through `self._project_decoder_layer`.
        ...
        # - Pass the sum through `torch.tanh` and then through `self._output_layer`.
        ...
        #   `attention` is a fixed-size representation for every batch element, independently of
        #   how many characters the corresponding input word had.
        # - Finally, concatenate `inputs` and `attention` (in this order), and call `self._cell`
        #   on this concatenated input and `states`, returning the result.
        return ...
...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        # TODO: Define
        # - `self._target_rnn_cell` as a `WithAttention` with `attention_dim=args.rnn_dim`, employing
        #   `torch.nn.GRUCell` with `args.rnn_dim` as the underlying cell. The cell will process concatenated
        ...
        if not args.tie_embeddings:
            # TODO(lemmatizer_noattn): Define `self._target_embedding` as an embedding layer of the target
    ...
    def encoder(self, words: torch.Tensor) -> torch.Tensor:
        ...
        # TODO: Run `self._source_rnn` on the embedded sequences, correctly handling
        # padding. Newly, the result should be the encoding of every sequence element,
    ...
    def decoder_training(self, encoded: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # the `setup_memory` method of `self._target_rnn_cell` on the `encoded` input.
        ...

        # TODO: Process the generated inputs by
        # - the `self._target_embedding` layer to obtain embeddings,
        # - repeatedly call `self._target_rnn_cell` on the sequence of embedded
        #   inputs and the previous states, starting with the state `encoded[:, 0]`,
        #   obtaining outputs for all target hidden states,
        # - `self._target_output_layer` to obtain logits,
        # - finally, permute dimensions so that the logits are in dimension 1,
    ...
    def decoder_prediction(self, encoded: torch.Tensor, max_length: int) -> torch.Tensor:
        ...
        # - `states`: the initial RNN state from the encoder, i.e., `encoded[:, 0]`.
        ...
        while index < max_length and torch.any(result_lengths == max_length):
            ...
            # - First embed `inputs` using the `self._target_embedding` layer.
            ...
            # - Pass the outputs through `self._target_output_layer`.
            # - Generate the most probable prediction for every batch example.
            predictions = ...

            # Store the predictions in `results` and update `result_lengths`
            # by setting it to the current `index` if an EOW was generated for the first time.

...
class CAGS:
        ...
        def _decode(self, data: dict, indices: dict, index: int) -> "CAGS.Element":
            return {
                ...
                "mask": torchvision.io.decode_image(
                    ...
                    torchvision.io.ImageReadMode.RGB).to(dtype=torch.float32).div(255).permute(1, 2, 0),
    ...
    def __init__(self, sizes: dict[str, int] | None = None, decode_on_demand: bool = False) -> None:
        for dataset, size in [("train", 2_142), ("dev", 306), ("test", 612)]:
            size = sizes[dataset] if sizes and dataset in sizes else size

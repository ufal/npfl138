...
class Decoder(torch.nn.Module):
    ...
    def reset(self, texts: torch.Tensor) -> None:
        # TODO: Similarly to `Attention.reset`, the `reset` method initializes the decoder
...
class Tacotron(npfl138.TrainableModule):
    ...
    def compute_loss(
        ...
        # TODO: We need to ignore the padding values during loss computation; therefore, use
        # `torch.masked_select` to select only the non-padding values from the predicted and true values.

        # TODO: The loss is the sum of the following three terms:

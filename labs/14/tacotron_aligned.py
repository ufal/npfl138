...
class Decoder(torch.nn.Module):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # TODO(tacotron): The pre-net is composed of `args.prenet_layers` layers, each consisting of:
        # - a linear layer with `args.prenet_dim` output dimension,
        # - ReLU activation,
        # - dropout with `args.dropout` rate.
        self.prenet = ...

        # TODO(tacotron): The post-net is composed of `args.postnet_layers` layers, each consisting of:
...
class Tacotron(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, num_characters: int) -> None:
        super().__init__()
        # TODO(tacotron): Create the Tacotron 2 model, which consists of the encoder, attention, and decoder modules.
        self.encoder = ...
        self.attention = ...
        self.decoder = ...
    ...
    def forward(self, texts: torch.Tensor, spectrograms_len: torch.Tensor) -> torch.Tensor:
        ...
        # TODO(tacotron): Finally, stack the `mel_frames` and `gates` lists into tensors.
        ...

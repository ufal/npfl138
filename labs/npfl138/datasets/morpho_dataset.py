    ...
    class Factor:
        ...
        def finalize(self, train: Self | None = None) -> None:
            ...
            if train:
                ...
                self.string_vocab = Vocabulary(strings, add_pad=True, add_unk=True)
...
if __name__ == "__main__":
    ...
    if args.evaluate:
        ...
        if args.task == "tagger":
            ...
            raise ValueError(f"Unknown task '{args.task}', valid values are only 'tagger' or 'lemmatizer'")

        with open(args.evaluate, "r", encoding="utf-8-sig") as predictions_file:
            accuracy = MorphoDataset.evaluate_file(gold, predictions_file)
        print(f"{args.task.title()} accuracy: {100 * accuracy:.2f}%")

...
"""The Street View House Numbers (SVHN) dataset annotates for every photo
of a house number all digits appearing on it, including their bounding boxes.

You can see a demo of the dataset [here](https://ufal.mff.cuni.cz/~straka/courses/npfl138/2526/demos/svhn_train.html).
...
- `"classes"`: a 1D `torch.Tensor`  with labels of all digits appearing in the image,
- `"bboxes"`: a `[num_digits, 4]` 2D `torch.Tensor` with bounding boxes of every
  digit in the image, each represented as `[TOP, LEFT, BOTTOM, RIGHT]` coordinates.

The dataset is split into:

- `train`: 10,000 images for training;
...
class SVHN:
    ...
    TOP: int = 0
    """The index of the top coordinate (y_min) in a bounding box."""
    LEFT: int = 1
    """The index of the left coordinate (x_min) in a bounding box."""
    BOTTOM: int = 2
    """The index of the bottom coordinate (y_max) in a bounding box."""
    RIGHT: int = 3
    """The index of the right coordinate (x_max) in a bounding box."""
    ...
    def evaluate(
        ...
        Returns:
          accuracy: The average accuracy of predicting a complete house number, i.e.,
            all digits and their bounding boxes must be correct.
        """
    ...
    def evaluate_file(gold_dataset: Dataset, predictions_file: TextIO) -> float:
        ...
        for line in predictions_file:
            values = line.split()
            if len(values) % 5:
                raise RuntimeError(f"Each prediction must contain multiple of 5 numbers, found {len(values)}")
    ...
    def visualize(image: torch.Tensor, labels: list[Any], bboxes: list[BBox], show: bool):
        """Visualize the given image plus the recognized objects.
...
if __name__ == "__main__":
    ...
    if args.evaluate:
        ...
        print(f"SVHN accuracy: {100 * accuracy:.2f}%")

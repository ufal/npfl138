...
- `train` containing 17,095 paragraphs with 58,483 questions,
- `dev` containing 1,473 paragraphs with 5,107 questions, and
- `test` containing 1,202 paragraphs with 4,900 questions.

Each split contains a list of paragraphs in the `paragraphs` field.

Each paragraph is a dictionary containing the following:

- `context`: a text of the paragraph as a string,
- `qas`: a list of questions and answers, each a dictionary with:
    - `question`: a text of the question,
    - `answers`: a list of answers, each answer a dictionary containing:
        - `text`: an answer text as string, exactly as appearing in the context,
...
class ReadingComprehensionDataset:
    ...
    def __init__(self, dataset: str = "reading_comprehension") -> None:
        """Load the dataset, downloading it if necessary.

        Parameters:
          dataset: The name of the dataset, for example `reading_comprehension`.
        """
        ...
        with zipfile.ZipFile(path, "r") as zip_file:
            for split in ["train", "dev", "test"]:
                with zip_file.open(f"{dataset}_{split}.txt", "r") as dataset_file:
                    setattr(self, split, self.Dataset(dataset_file))
...
if __name__ == "__main__":
    ...
    if args.evaluate:
        ...
        print(f"Reading comprehension accuracy: {100 * accuracy:.2f}%")

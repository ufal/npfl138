...
The input images are grayscale, they have arbitrary height and width, and they
represent a single stave of music. The annotations are “semantic” (the notes are annotated
with their real pitch and duration which depend on current clef and key signature).

The dataset is automatically downloaded if necessary, but note that is has
500MB, so it might take a while. You can see the first 100 training examples
[here](https://ufal.mff.cuni.cz/~straka/courses/npfl138/2526/demos/homr_train.html).

Each dataset element is a Python dictionary with the following keys:

- `"image"`: a grayscale image of arbitrary size stored as `torch.uint8` values in [0-255] range
  in a tensor of shape `[1, HEIGHT, WIDTH]`,
- `"marks"`: a tensor of integers representing the marks in the image.

The dataset is split into:

- `train`: 51,365 images for training;
- `dev`: 5,027 images for development (validation);
- `test`: 5,023 images for testing.
"""
...
class HOMRDataset:
    ...
    def __init__(self, decode_on_demand: bool = False) -> None:
        ...
        for dataset, size in [("train", 51_365), ("dev", 5_027), ("test", 5_023)]:
            path = download_url_to_file(self.URL, f"homr.{dataset}.tfrecord")
...
HOMRDataset.MARKS_VOCAB = Vocabulary(HOMRDataset.MARK_NAMES)
...
if __name__ == "__main__":
    ...
    if args.evaluate:
        ...
        print(f"HOMR edit distance: {100 * edit_distance:.3f}%")

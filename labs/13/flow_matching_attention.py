...
class SelfAttentionBlock(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if npfl138.first_time("SelfAttentionBlock.forward"):
            print(f"torch.var of the first batch returned by SelfAttentionBlock: {torch.var(result):.4f}")
        return result
...
class DownscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int, attention_heads: int | None = None) -> None:
        super().__init__()
        # TODO(flow_matching): Create layers of the downscaling block. The downscaling block
        # starts with `residual_blocks` number of `ResidualBlock`s.
        # TODO: Then, if `attention_heads` is not None, a SelfAttentionBlock with
    ...
    def forward(self, images: torch.Tensor, times: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # the input images and times through the residual blocks and (if present) the self-attention block,
...
class UpscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int, attention_heads: int | None = None) -> None:
        ...
        # TODO: Lastly, if `attention_heads` is not None, a SelfAttentionBlock with
        # `width` channels and `attention_heads` heads is called.
        ...
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, channels: int, stage_blocks: int, stages: int, attention_stages: int, attention_heads: int):
        super().__init__()
        # TODO(flow_matching): Create layers of the U-Net architecture block. Start by
        # creating a `SinusoidalEmbedding` layer with dimension `channels`.
        #
        ...
        #   a self-attention block with `attention_heads` heads;

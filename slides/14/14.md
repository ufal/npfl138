...
fixed-length spectral densities (usually power densities, i.e., $x^2$).
...
~~~
The location features are computed using 32 1D convolutions with kernel size 31,
and the attention projects the current inputs and location features to 128
dimensions.
...
~~~
Therefore, a separate _attention RNN_ is used, which processes the pre-net
concatenated with attention context vector $→α_i^T →h$, and its output is used
as $→s_{i-1}$ in the attention computation.
...
The decoder predicts the spectrogram one frame at a time. The predictions from
the previous step are first passed through a _pre-net_ composed of
2 fully-connected ReLU layers with 256 units and concatenated with attention
context vector. The decoder consists of 2 1024-dimensional LSTM cells and its
output is linearly projected to the predicted frame.

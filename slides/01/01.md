...
machine learning model, especially when the data is high-dimensional (images,
videos, speech, texts, …).
...
A probability distribution describes how likely the individual values that
a random variable can take are.
...
Self-information can be considered as the amount of **surprise** when a random variable is sampled.
~~~
...
- reasonably compact,
- allow heavy parallelization during execution (GPUs, TPUs, …).

...
process in reverse.
~~~
We therefore teach a model $p_{→θ}(⁇→x_{t-1}|⁇→x_t)$ to approximate the reverse
of $q(⁇→x_t|⁇→x_{t-1})$, and obtain the _reverse process_:
...
The normal (or Gaussian) distribution is a continuous distribution parametrized by
...
~~~
However, in this lecture we will only consider _isotropic_ distributions, where $⇉Σ = σ^2⇉I$:
...
- The sum of two independent normally-distributed random variables $⁇→x_1 ∼ 𝓝(→μ_1, σ_1^2⇉I)$
  and $⁇→x_2 ∼ 𝓝(→μ_2, σ_2^2⇉I)$ follows the normal distribution $⁇𝓝\big(→μ_1 + →μ_2, (σ_1^2 + σ_2^2)⇉I\big)$.
...
~~~
Therefore, if $ᾱ_t → 0$ as $t → ∞$, then the $⁇→x_t$ converges to $𝓝(→0, ⇉I)$ as $t → ∞$.
...
Originally, a linearly increasing sequence of noise variances
...
~~~
In practice, we want to avoid both the values 0 and 1, and keep $α_t$ in the $[ε, 1-ε]$ range.
...
\end{aligned}$$
corresponds to a uniform movement on this arc.
...
In order to generate images, we teach the model $p_{→θ}(⁇→x_{t-1}|⁇→x_t)$
to approximate the reverse of $q(⁇→x_t|⁇→x_{t-1})$.
...
for some fixed sequence $σ_1, …, σ_T$.
...
~~~
$\displaystyle\kern2em{} ∝ \exp\Bigg(-\frac{1}{2}\Bigg(\textcolor{purple}{\frac{(⁇→x_t - \sqrt{α_t}⁇→x_{t-1})^2}{β_t}} + \textcolor{darkcyan}{\frac{(⁇→x_{t-1} - \sqrt{ᾱ_{t-1}}⁇→x_0)^2}{1-ᾱ_{t-1}}} - \textcolor{red}{\frac{(⁇→x_t - \sqrt{ᾱ_t}⁇→x_0)^2}{1-ᾱ_t}}\Bigg)\Bigg)$
...
~~~
The authors found out that training without the weighting term performs better, so
the final loss is
...
~~~
In practice, instead of being discrete, $t$ may be continuous in the $[0, 1]$ range.
~~~
...
of these approaches, and it is the method we now describe in detail.
...
~~~
The modern approach to generative modeling is to start with a simple base
distribution $p_0$, usually a standard Gaussian $𝓝(→0, ⇉I)$, and learn
a mapping that transforms this distribution into $p_\textrm{data}$.

![w=60%,h=center](pushforward.svgz)

~~~
Sampling then can be performed by drawing from $p_0$ and applying the
transformation.
...
value means a point is a source; negative means it is a sink).
...
and the $u_t$ should look like given that there are many possible probability
...
$u_t(→x)$.
~~~
Usually, a U-Net architecture with skip connections is used to model $v_t$.

~~~
### Training

During training, we minimize the objective corresponding to flow matching
$$𝓛_\textrm{FM}(→θ) ≝ 𝔼_{t, →x∼p_t(→x)} \|v_t(→x; →θ) - u_t(→x)\|^2,$$
...
perform numerical integration by the Euler method with $T$ steps:
...
as a mixture of simpler probability paths. Given the fact that
...
conditions; we assume that all our objects are “nice enough”.
...
which is an affine transformation mapping the standard normal distribution to
...
~~~
Recall the flow ODE $\tfrac{d}{dt} φ_t(→x) = u_t(φ_t(→x))$. This equation could be used
to define $u_t$, but unfortunately only along the trajectory $φ_t(→x)$, not for arbitrary $→x$.

~~~
However, the affine map $φ_t$ has an analytical inverse (assuming $σ_t(→x_1) > 0$)
$$\textcolor{blue}{φ_t^{-1}(→z)} = \textcolor{blue}{\frac{→z - μ_t(→x_1)}{σ_t(→x_1)}}.$$

~~~
Therefore, when we consider $→z = φ_t(→x)$, we get $u_t(→z | →x_1) = \textcolor{darkred}{φ'_t(\textcolor{blue}{φ_t^{-1}(→z)})}$.
...
~~~
for our specific case of Optimal Transport flow we obtain
...
During training, we minimize the CFM objective using SGD/Adam:
$$𝓛_\textrm{CFM}(→θ) ≝ 𝔼_{t, →x_1 ∼ p_\textrm{data}, →x_0 ∼ p_0} \big\|v_t\big(φ_t(→x_0); →θ\big) - \big(→x_1-(1-σ_\textrm{min})→x_0\big)\big\|^2.$$
...
- Additionally, on several lower-resolution levels, a self-attention
  ...
  Because the complexity scales asymptotically as the fourth power of the image
  width, only the lower-resolution levels are used for this self-attention.
...
- an image (e.g., for super-resolution): the image is then resized and
  concatenated with the input noisy image (and optionally in other places,
  like after every resolution change);

~~~
- a text: the usual approach is to encode the text using some pre-trained
  encoder, and then introduce an “image-text” attention layer (usually
...
~~~
- During sampling, we pronounce the effect of conditioning by taking the
  unconditioned vector field and adding the difference between the conditioned and
  unconditioned vector fields, _weighted by the weight_ $w$ (values like $w=5$ or
...
- $L_t = D_\textrm{KL}\big(q(⁇→x_{t-1}|⁇→x_t,⁇→x_0) \| p_{→θ}(⁇→x_{t-1}|⁇→x_t)\big)$
  is the KL divergence between two Gaussians, so it can be computed explicitly as
...
~~~
$\displaystyle\kern5em{} = \Bigg(\frac{\sqrt{ᾱ_{t-1}}β_t}{1-ᾱ_t}\frac{1}{\sqrt{ᾱ_t}} + \frac{\sqrt{α_t}(1-ᾱ_{t-1})}{1-ᾱ_t}\Bigg)⁇→x_t - \Bigg(\frac{\sqrt{ᾱ_{t-1}}β_t}{1-ᾱ_t}\frac{\sqrt{1-ᾱ_t}}{\sqrt{ᾱ_t}}\Bigg)⁇→e_t$

~~~
$\displaystyle\kern5em{} = \frac{β_t + α_t(1-ᾱ_{t-1})}{(1-ᾱ_t)\sqrt{α_t}}⁇→x_t - \Bigg(\frac{β_t}{\sqrt{1-ᾱ_t}\sqrt{α_t}}\Bigg)⁇→e_t = \textcolor{blue}{\frac{1}{\sqrt{α_t}}\Bigg(⁇→x_t - \frac{1-α_t}{\sqrt{1-ᾱ_t}}⁇→e_t\Bigg)}.$
...
the training algorithm is exactly identical to DDPM, only the sampling algorithm
is different.
...
presented here is used almost exclusively in practice.
...
theorem and the above definition, but we do not actually need it.
...
a uniform subsequence $T, …, 1$ is used), and let $t_0=0$.
~~~
Starting from the initial noise $⁇→x_T ∼ 𝓝(→0, ⇉I)$, we perform $S$ sampling
...
Recall that log-likelihood-based models explicitly represent the density
...
When $ε → 0$ and $K → ∞$, $⁇→x_K$ obtained via the Langevin dynamics
...
regions, we perturb the data distribution by an isotropic Gaussian noise
...
is analogous to the forward process in the diffusion models.
...
so the score function basically estimates the noise given a noisy image.

---
# Noise Conditional Score Network

Once we have trained the score function for various noise rates $σ_t$, we can
sample using annealed Langevin dynamics, where we utilize gradually
...
~~~
Such a procedure is reminiscent of the sampling step in the reverse diffusion process.

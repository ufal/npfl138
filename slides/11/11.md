...
_Next sentence prediction_ was originally hypothesized to be an important
factor during training of the BERT model, as indicated by ablation experiments.
However, later experiments indicated removing it might improve the results.
...
- SEGMENT-PAIR: a pair of segments with at most 512 tokens in total;
~~~
- SENTENCE-PAIR: a pair of _natural sentences_, usually significantly
...
BERT is trained for 1M steps with learning rate 1e-4.
...
Multilingual BERT is pre-trained on 102–104 largest Wikipedias, including the
Czech one.

~~~
There are two versions: the _cased_ one uses a WordPiece vocabulary that preserves case,
while the _uncased_ one uses only lowercase subwords _without diacritics_.

~~~
Although only a very small percentage of input sentences were Czech, it works
...
a question, the answer needs to be located in the paragraph.

Training the model on English and then directly running it on a different
...
Transformer-based language models have demonstrated remarkable performance,
...
have been repeatedly proposed in the literature. Such relative positional embeddings
are applied to the query-key similarity $→q_m^\T→k_n$ and depend only on the
difference $m-n$, not on $m$ or $n$ directly.

~~~
One prominent variant is **rotary positional embeddings** (RoPE), which have become
the de-facto standard positional embeddings in large language models.

~~~
They do not require trainable parameters, they scale remarkably well to context
windows of 128k+ tokens, and the models trained with RoPE can be finetuned on much
longer sequences at just a fraction of the original compute.
...
First assume that the queries and the keys are just 2D vectors, which we interpret as
complex numbers. The idea of RoPE is to multiply a query $q_m$ by $e^{imθ}$
and a key $k_n$ by $e^{inθ}$. Then the cosine of the angle between these two vectors
is
...
In reality, we do not compute with complex numbers, but use 2D rotation
matrices:
$$\begin{gathered}
...
To extend the idea to $d$-dimensional embeddings, we consider them as
$d/2$-dimensional complex vectors and apply the 2D transformation to
...
The original approach relied on the so-called Reinforcement Learning from Human Feedback (RLHF); lately, other
...
proposed processing images with ViT – a variant of the Transformer
...
Improved training using a variety of data augmentations (DeiT architecture,
Dec 2020) resulted in performance close to EfficientNet when trained only
on the ImageNet1k dataset (83.1% vs 84.7% top-1 accuracy).

~~~
_DeiT III: Revenge of the ViT_ (Apr 2022) has presented a simplified training
...
is BERT-like masking, which was proposed in the Nov 2021 paper
...
~~~
This MAE architecture reaches 86.9% top-1 accuracy with ImageNet1k-only training
on images of size 224, and 87.8\% on images of size 448.

---
# Image Recognition with Transformers

In April 2025, the second best model on ImageNet was BASIC-L trained with the
Lion optimizer. The image encoder in BASIC-L is CoAtNet-7, an architecture
combining MBConv in the first stages and relative pre-activated 2D self-attention in
the later stages, with GELU activations everywhere. The image encoder has 2.4B
parameters, it is trained on 6.6B noisy image-text pairs using a batch size
of 65,536 images in ~7k TPUv4-days, and achieves 91.1% top-1 accuracy.
...
domain-specific Transformer-based encoders and then shared
...
afterward (92.4% top-1 ImageNet accuracy).
...
_Develop a goal-seeking agent trained using a reward signal._
...
just the immediate reward, but also all the following rewards.
...
~~~
- Improving efficiency of the VP9 codec (2022; 4% in bandwidth with no loss in
  quality)
...
- Designing the layout of TPU chips (AlphaChip; since 2021, open-sourced)
...
obtaining $R_2$, selecting $A_3$, …, with the indices corresponding to the time step
...
of action $a$ at time $t$ (before taking the $t$-th trial).
...
Choosing the greedy action corresponds to **exploitation** of the current estimates.
We however also need to **explore** the space of actions to improve our estimates.
...
~~~
The _$ε$-greedy_ method follows the greedy action with probability $1-ε$, and
...
~~~
- $p(S_{t+1} = s', R_{t+1} = r | S_t = s, A_t = a)$ is the probability that action $a ∈ 𝓐$
  taken in state $s ∈ 𝓢$ will lead to state $s' ∈ 𝓢$, producing **reward** $r ∈ ℝ$,
  the value of which is bounded.
~~~
- $γ ∈ [0, 1]$ is a **discount factor** (we always use $γ=1$ and finite episodes in this course).

~~~
Let $G_t ≝ ∑_{k=0}^∞ γ^k R_{t + 1 + k}$ denote the **return**. The goal is to optimize $𝔼[G_0]$.
...
$H$ steps. These **finite-horizon tasks** can then use discount factor $γ=1$,
because the return $G ≝ ∑_{t=0}^H γ^t R_{t + 1}$ is well defined.

~~~
If the agent–environment interaction continues indefinitely, we instead
...
A **policy** $π$ computes the distribution of actions in a given state, i.e.,
$π(a | s)$ corresponds to the probability of performing action $a$ in state $s$.

~~~
We model the policy as a neural network with parameters $→θ$:
$$π(a | s; →θ).$$

~~~
If the number of actions is finite, we consider the policy to output a categorical
...
$$v_*(s) ≝ \max_π v_π(s),$$
~~~
and the **optimal action-value function** is defined analogously as
...
Assume that $𝓢$ and $𝓐$ are finite, $γ=1$, and that the maximum episode length $H$ is also finite.
...
~~~
However, the sum over all actions is problematic. Instead, we rewrite it into an
...
REINFORCE therefore minimizes the loss $-J(→θ)$ using the gradient
...
A good choice for $b(s)$ is $v_π(s)$, which can be shown to in some sense
minimize the variance of the gradient estimator (see L. Weaver and N. Tao,
_The Optimal Reward Baseline for Gradient-Based Reinforcement Learning_,
https://arxiv.org/abs/1301.2315, for details).
Such a baseline corresponds to centering the returns, given that
...
is to approximate it by another neural network model. This model is trained
using mean squared error of the predicted and observed returns.
...
~~~
- Each block is designed by an RNN controller generating 5 parameters.
...
- Every block is designed by an RNN controller generating individual operations.
...
~~~
In total, 8k model architectures are sampled, and the PPO algorithm is used

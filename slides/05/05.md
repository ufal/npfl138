...
The authors considered several alternatives, and chose the one in which
a $1×1$ convolution + BN is applied to the projections whenever the
required number of channels needs to be increased. The required spatial
resolution is achieved by using stride 2.
...
$D_k = \lfloor D_{k-1} + α/N\rfloor$, where $D_k$ is the number of filters
in the $k$-th convolutional block out of $N$ and $α$ is the number of filters
to add in total.
...
to zero-pad the missing channels, where the zero-padded channels correspond
...
~~~
All $p_l$ can be set to a constant, but a more effective approach is to utilize
...
Drop a $16×16$ square in the input image, with a randomly chosen center.
...
~~~
- We sample a bounding box center uniformly.
~~~
...
~~~
A substantially improved automatically-generated EfficientNet was released two
years later.
...
- **excitation (adaptive recalibration)** computes a weight for every channel
  ...
  with it. To avoid increasing the number of parameters too much (by $C^2$), an
...
- There is no nonlinear activation on the bottlenecks (it would lead to
  loss of information given the small capacity of bottlenecks).

---
# Mobile Inverted Bottleneck Convolution

The mobile inverted bottleneck convolution is denoted for example as _MBConv6 k3x3_,
where the 6 denotes the expansion factor after the bottleneck and $3×3$ is the
...
In 2019, a very performant and efficient convolutional architecture
...
The resulting network is referred to as the **EfficientNet-B0** baseline network.
...
epochs. Dropout with rate 0.2 is used on the last layer, stochastic
...
~~~
By a grid search on a network with double the computational complexity, the best
...
published. It is currently one of the very good CNNs available for image recognition.
...
- The separable convolutions have fewer parameters, but are slow to execute on
  ...
  parameters and requires more computation, but is in fact executed faster
...
~~~
The simplest scenario is to take an ImageNet model, drop the last classification
...
~~~
For efficiency, we may precompute the image features **once** and reuse them later
many times.
...
So far, the convolution operation produced either an output of the same size,
or it produced a smaller one if stride was greater than one.

~~~
In order to come up with an **upscaling convolution**, we start by considering how
...
~~~
  This operation is called a **transposed** or **upscaling** convolution and

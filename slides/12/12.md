...
We assume $P(⁇→z)$ is fixed and independent of $⁇→x$.
...
~~~
However, in order to train an autoencoder, we need to know $P_{→θ}(→z | →x)$,
the “inverse”, which usually cannot be computed directly.
...
Recall that convex functions by definition fulfill that for $→u, →v$ and real $0
≤ t ≤ 1$,
$$f(t→u + (1-t)→v) ≤ tf(→u) + (1-t)f(→v).$$

~~~
**Jensen's inequality** generalizes the above property to any _convex_
...
~~~
Jensen's inequality can also be formulated for probability distributions
...
Our goal is to maximize the log-likelihood as usual, but we need to express
...
~~~
We therefore turn to our _encoder_, which, for a given $→x$, is able to generate
“its” $→z$:
...
~~~
$\displaystyle \kern6em{} = \log 𝔼_{Q_{→φ}(→z|→x)} \bigg[P_{→θ}(→x | →z) ⋅ \frac{P(→z)}{Q_{→φ}(→z|→x)}\bigg]\kern2.7em\textcolor{gray}{\textrm{\footnotesize now we use Jensen's inequality}}$
...
denoted $𝓛(→θ, →φ;⁇→x)$, can also be defined explicitly as
...
~~~
  - In order for $→σ$ to be positive, we can use the $\exp$ activation function
...
~~~
- **latent loss** – over all $→x$, the distribution $Q_{→φ}(→z | →x)$ should be as close as
  possible to the prior $P(→z) = 𝓝(→0, →I)$, which is independent of $→x$.
...
~~~
Such a formulation then allows differentiating $→z$ with respect to
$→μ$ and $→σ$ and is called the **reparametrization trick** (Kingma and Welling, 2013).
...
---
# Variational AutoEncoders – Excessive Latent Loss

![w=50%,h=center](vae_high_latent_loss.png)

---
# Variational AutoEncoders – Excessive Reconstruction Loss
...
~~~
Then we have a **discriminator** $D(→x; →θ_d)$, which given data $→x$ generates the probability
whether $→x$ comes from real data or is generated by the generator.

~~~
The discriminator and the generator play the following game:
...
The generator and the discriminator are trained alternately, the discriminator using
$$\argmax_{→θ_d} 𝔼_{→x ∼ P_\textrm{data}}[\log D(→x)] + 𝔼_{→z ∼ P(⁇→z)}[\log (1 - D(G(→z)))]$$
and the generator via
...
where the discriminator can easily distinguish between real and generated samples,
the generator can be trained using
$$\argmin_{→θ_g} 𝔼_{→z ∼ P(⁇→z)}[-\log D(G(→z))]$$
instead, which results in the same fixed-point dynamics, but much stronger
gradients early in the learning process.
...
so the SGD will diverge with arbitrarily small step size.
...
~~~
- Unrolling the discriminator update helps the generator consider not just the
...
~~~
In the past few years, GANs saw a big development, improving the sample quality
substantially.
~~~
...
~~~
- An autoregressive sampling is used in VQ-VAE (PixelCNN architecture).
...
- In VQGAN, a Transformer is used to model the discrete latent variables.
...
Instead of minimizing the JS divergence
$$\operatorname{JSD}(p \| q) = \frac{1}{2} D_\textrm{KL}\big(p \| (p+q)/2\big) + \frac{1}{2} D_\textrm{KL}\big(q \| (p+q)/2\big),$$
...
from $x$ to $y$, and EMD is the “cost” of the optimal transport plan.

---
class: dbend
# Wasserstein GAN

The EMD behaves much better than the JSD.
...
so the discriminator returns a single output without an activation and it needs to be 1-Lipschitz.
...
We could aim for a maximum margin classifier by using Hinge loss, where we update the
discriminator using
...
Because convolutions process local information only, a non-local _self-attention_
...
The so-called **truncation trick** is used to trade off fidelity and variety
– during training, $→z$ is sampled from $𝓝(→0, →I)$, while during generation
it is sampled from a _truncated normal_ distribution.

In the following example, samples were generated using thresholds $2, 1, 0.5, 0.04$.

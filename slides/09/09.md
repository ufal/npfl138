...
Usually, the generated logits are processed by an $\argmax$, the chosen word is
embedded and used as the next input.
...
- embed the previous prediction, using a matrix in $ℝ^{V × D}$, where
  $V$ is the vocabulary size and $D$ is the embedding size;
- classify the hidden state into current prediction, using a matrix in
  $ℝ^{D × V}$.

~~~
Both these matrices have a similar meaning – they represent words in the embedding
...
them as the transposition of the other.
~~~
...
As another input during decoding, we add a _context vector_ $c_i$:
$$→s_i = f(→s_{i-1}, →y_{i-1}, →c_i).$$

~~~
We compute the context vector as a weighted combination of the source sentence
encoded outputs:
$$→c_i = ∑_j α_{ij} →h_j$$

~~~
The weights $α_{ij}$ are the softmax of $e_{ij}$ over $j$,
$$→α_i = \softmax(→e_i),$$
with $e_{ij}$ being
$$e_{ij} = →v^\T \tanh(⇉V→h_j + ⇉W→s_{i-1} + →b).$$
...
We translate **subword units** instead of words. The subword units can be generated
in several ways, the most commonly used are:

~~~
- **BPE**:
  Using the _byte pair encoding_ algorithm. Start with individual characters plus
  a special end-of-word symbol $•$. Then, merge the most frequent symbol pair
  $A, B$ into a new symbol $AB$, with the symbol pair never crossing word boundary
  (so that the end-of-word symbol cannot be inside a subword).

~~~
  Considering a text with the words _low, lowest, newer, wider_, a possible sequence
  of merges is
...
- However, in existing implementations, we usually represent
  a **beginning-of-word** symbol at the beginning of a subword
  instead (usually a suitably encoded space).
~~~
- Furthermore, when considering multilingual models, there are in fact quite
  ...
  - Byte-level BPE or BBPE allows even subword splitting at arbitrary
    boundaries (inside a single UTF-8-encoded codepoint). **ChatGPT**, **Llama 3+**
~~~
  - A sentence-piece model splits at Unicode codepoint boundaries, but can represent
    unknown characters using bytes of the UTF-8 encoding. **Gemini**
~~~
- For example, the phrase “Přelétavý motýlek” can be tokenized as
...
---
# Subword Units – WordPiece

- **WordPiece**:
  Given a text divided into subwords, we can compute the unigram probability of
...
~~~
  ...
  _end-of-word_ character), and then repeatedly join the pair of subwords
...
often generated from the union of the two vocabularies of the source and target
languages (the so-called _joint BPE_ or _shared wordpieces_).

---
# BPE and WordPiece Comparison

Both BPE and WordPiece were being used originally; the biggest
...
- for BPE, the sequence of merges must be performed in the same order as during
  ...
  data);
~~~
- for WordPiece, it is enough to find the longest matches from the subword
...
~~~
Nowadays, most large language models have adopted the BPE approach.

~~~
Both algorithms are implemented in quite a few libraries, most notably the
`sentencepiece` library, the Hugging Face `tokenizers` package, and the OpenAI
`tiktoken` package.
...
There were many attempts at multilingual translation.
...
By unsupervised we understand a setting where we have access to large
...
~~~
Nowadays, language models like ChatGPT can also be considered unsupervised
...
~~~
Instead, we may want to combine sequence elements independently of
their distance.

~~~
Such processing is possible in the **Transformer** architecture, originally
proposed for neural machine translation in 2017 in the _Attention is All You Need_
paper.
...
~~~
$$\textrm{Attention}(⇉Q, ⇉K, ⇉V) = \softmax\left(\frac{⇉Q ⇉K^\T}{\sqrt{d_k}}\right)⇉V.$$
...
In practice, multihead attention is used. Instead of using one huge attention, we
split queries, keys and values into several groups (similar to how ResNeXt works),
compute the attention in each group separately, concatenate the
results and multiply them by the matrix $⇉W^O$.
...
The self-attention is complemented by FFN layers, which consist of a fully connected
...
In the encoder-decoder attentions, the _queries_ come from the decoder, while the
...
~~~
The default dropout rate and the label smoothing weight are both 0.1.
...
The Adam optimizer (with $β_2=0.98$, smaller than the default value of $0.999$)
...
$\textit{warmup\_steps}$ updates, the learning rate increases linearly from
...
~~~
Note that the main goal of the warmup is to prevent divergence early in training;
...
The subwords were constructed using BPE with a shared vocabulary of about 37k tokens.

---
# Transformers Variations on En→De newtest2014 Dev
...
In the seq2seq architecture, we have both an **encoder** and a **decoder**.
...
~~~
- They consist purely of the decoder part of a Transformer (so they contain
  neither an encoder nor encoder-decoder attention; consequently, all their
  self-attentions are masked).
...
- It is usually trained using **teacher forcing**, and uses **autoregressive decoding**.

- **Attention** allows focusing on any part of the sequence in every time step.

- Compared to **RNNs**, the **Transformer** provides a more powerful sequence-to-sequence
  architecture, as well as strong sequence element representations, but requires

...
~~~
- We can decide to ignore it and use heuristics capable of recovering the spans
...
- The most probable sequence has logprob $\max α_N$, last element $y_N ← \argmax α_N$, and the other elements can be recovered
  by traversing $β$ from $t=N$ downto $t=2$.
</div>
...
~~~
You can have a look at https://distill.pub/2017/ctc/ for a nice and detailed
description.
...
- Additionally, if we have a whole sequence of log probabilities $→a$, we can sum
  all of them using `torch.logsumexp`, `keras.ops.logsumexp`.

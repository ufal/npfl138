...
A bounding box is parametrized as follows. Let $x_r, y_r, w_r, h_r$ be the
center coordinates and the width and height of the RoI respectively, and let $x, y, w, h$ be
the parameters of the bounding box. We represent the bounding box relative
...
~~~
In Fast R-CNN, the smooth $L_1$ loss, or **Huber loss**, is employed for bounding box parameters:
...
In the datasets, the bounding boxes are usually represented using the `XYXY` format:
...
with ground-truth boxes; the others are chosen to have the IoU in the range $[0.1, 0.5)$,
...
boxes (or masks) in the order of the ranks and generate a precision/recall curve,
considering a bounding box as correct if it has an IoU of at least 0.5 with any
ground-truth box.
...
Finally, the average precision for a single class is the average of the precision values
at recall levels $0.0, 0.1, 0.2, …, 1.0$.

~~~
The final AP is the mean of average precision of all classes.
...
~~~
In the original metric, an IoU of 50% is enough to consider a prediction valid.
We can generalize the definition to $\mathit{AP}_{t}$, where an object
prediction is considered valid if the IoU is at least $t$%.
...
~~~
The region proposal network produces the so-called **region proposals**,
which then serve as the RoIs in the rest of the pipeline (i.e.,
the Fast R-CNN).
...
For every anchor, we classify the image region into two classes (background, object)
and also predict the region proposal bounding box relative to the anchor,
...
- furthermore, a positive example is also any anchor with
  an IoU of at least 0.7 for any ground-truth box;
~~~
- negative training examples for every anchor that has an IoU of at most 0.3
  with all ground-truth boxes;
~~~
...
top-scoring regions (i.e., the ones with the highest probability from the
...
A straightforward extension of Faster R-CNN able to produce image segmentation
...
More precise alignment of the RoI is required to accurately predict the masks.
...
The masks are predicted in a third branch of the object detector.

- A higher mask resolution is usually needed (at least $14×14$, or even more).
...
~~~
- Testing applicability of the Mask R-CNN architecture.
...
We employ an FPN as a backbone in Faster R-CNN.

~~~
For a ResNet-like network with $224×224$ input, we denote by $C_2, C_3, …, C_5$
the image features of the last convolutional layer of size $56×56, 28×28, …,
7×7$ (i.e., $C_i$ indicates downscaling by $2^i$).
~~~
...
$32^2, 64^2, 128^2, 256^2$, respectively). However, three different aspect
ratios $(1:1, 1:2, 2:1)$ are still used.
...
identified as the main issue preventing obtaining a performance comparable to
...
introduce a weighting factor $α ∈ (0, 1)$ for one class and $1 - α$ for the other
class, arriving at
...
~~~
Even if weighting focuses more on the low-frequent class, it does not distinguish
between easy and hard examples, contrary to focal loss.

~~~
In practice, focal loss is usually used together with class weighting:
...
~~~
Note that ResNet provides only the features $C_3$ to $C_5$. $C_6$ is computed
...
During training, anchors are assigned to ground-truth object boxes if the IoU is
at least 0.5; to background if the IoU with any ground-truth region is at most 0.4
(the rest of the anchors are ignored during training).
~~~
The classification head is trained using focal loss with $γ=2$ and $α=0.25$ (but
according to the paper, all values of $γ$ in the $[0.5, 5]$ range work well); the
boundary regression head is trained using the $\textrm{smooth}_{L_1}$ loss as in
Fast(er) R-CNN.
...
---
# RetinaNet – Variations

Variations use a ResNet-50-FPN backbone trained and tested with 600-pixel images.
...
In multi-scale fusion in an FPN, information flows only from the pyramid levels
...
to the same resolution and sum them – therefore, all the features are
...
- **Softmax-based fusion**: In each BiFPN node, we create a trainable weight
  $w_i$ for every input $⇶I_i$ and the final combination (after resizing, before
  a convolution) is
  $$∑_i \frac{e^{w_i}}{∑\nolimits_j e^{w_j}} ⇶I_i.$$

~~~
- **Fast normalized fusion**: Authors propose a simpler alternative way of
  weighting:

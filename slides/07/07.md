...
Given an input $→x^{(t)}$ and a previous state $→h^{(t-1)}$, the new state is computed as
$$→h^{(t)} = f(→h^{(t-1)}, →x^{(t)}; →θ).$$
...
~~~
If $⇉U$ has the eigenvalue decomposition $⇉U = ⇉Q ⇉Λ ⇉Q^{-1}$, we get that
$$→h^{(t)} ≈ ⇉Q ⇉Λ^t ⇉Q^{-1} →h^{(0)}.$$
...
~~~ ~~
They propose to achieve this by a _constant error carrousel_.
...
They also propose **input** and **output** gates which control the flow
...
information from the memory cell $→c_t$.
...
~~~
The formulas above are from the original paper; however, all GPU-accelerated
...
~~~
- GRU are easier to work with, because the state is just one tensor, whereas it is
...
- However, there are some tasks on which LSTM achieves (much) better results
  than GRU.
~~~
  - For a demonstration of the difference in the expressive power between LSTM and GRU
...
~~~
  - For a clear difference between LSTM and GRU on a real-world task, see for example
...
assuming the eigenvalue decomposition $⇉U = ⇉Q ⇉Λ ⇉Q^{-1}$, we get
$$→h^{(t)} ≈ ⇉Q ⇉Λ^t ⇉Q^{-1} →h^{(0)}.$$
...
- initialize biases to zero, except for the forget cell bias in LSTM
  initialized to 1.
...
For input $→x$, a fully connected layer computes
$$→y ← H(→x, ⇉W_H).$$

~~~
Highway networks add a residual connection with gating:
...
~~~
  - In PyTorch, `torch.nn.{RNN,LSTM,GRU}` has a `dropout` parameter, which adds
    a dropout layer with given dropout probability on the outputs of _all but the
    last RNN layer_, i.e., in positions where you cannot place it manually.
~~~
...
Dropout is applied only to the candidate states (i.e., the values added to the memory
cell in LSTM and to the previous state in GRU), independently at every time step.
...
would be position-invariant (the same as batch normalization is);
therefore, we would like parameters only for the channels, i.e., $→β ∈ ℝ^C$ and
$→γ ∈ ℝ^C$, but computing $μ$ and $σ^2$ over the whole image. However, most
...
~~~
Note that the architecture can be interpreted as a variant of a mobile inverted
...
We might represent **words** using one-hot encoding, treating all words as
independent.
...
~~~
Ideally, we would like the word representations to capture these similarities
...
~~~
We therefore represent words as fixed-size **embeddings** into the $ℝ^d$ space,
with the vector elements playing the role of the common underlying factors.
...
When a matrix is multiplied by a one-hot encoded vector (all but one zeros
...
Although the embedding layer is just a fully connected layer on top of one-hot
...
is to use RNNs: we embed _characters_ to get character representations,
and then use an RNN to produce a representation of the whole _sequence of
characters_.
...
is a level of the highest match of the pattern anywhere in the word.
...
~~~
- Copy the resulting embeddings back to their original positions in the batch.

...
~~~
  - In PyTorch, `torch.nn.{RNN,LSTM,GRU}` has a parameter `dropout`, which adds
    a dropout layer with given dropout probability on _all but last RNN layers_.
~~~
    - However, note that using a single multi-layer `torch.nn.{RNN,LSTM,GRU}`
      does not use residual connections, so I never use it.

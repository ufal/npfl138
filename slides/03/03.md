...
~~~
$\displaystyle \phantom{\kern5em\frac{∂L}{∂z_i} = \frac{∂}{∂z_i} \left[-\log \frac{e^{z_\textit{gold}}}{∑_j e^{z_j}}\right]}
...
- In reproduction, evolution is achieved using gene swapping. The genes must not
  be just good with combination with other genes, they need to be independently
  good.
...
`keras.optimizers.Optimizer`), **per variable** (`clipnorm`) or for the gradient as

...
For functions $x$ and $w$, _convolution_ $w \ast x$ is defined as
...
~~~
Closely related is _cross-correlation_, where the matrix $⇉K$ is flipped:
$$(⇉K \star ⇉I)_{i, j} = ∑\nolimits_{m, n} ⇉I_{i+m, j+n} ⇉K_{m, n}.$$
...
~~~
Batch normalization $BN$ is added just before a nonlinearity $f$, and it is useless

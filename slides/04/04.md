...
~~~
These channels have no longer any spatial structure, so $⇶K$ then contains
a different set of weights for every input dimension, obtaining
...
~~~
Such a three-dimensional $⇶K$ is usually called a convolutional **filter**.
...
---
style: .katex-display { margin: .5em 0 } li ul { margin-bottom: 0 }
# Convolution Layer

To define a complete convolution layer, we need to specify:
- the width $W$ and the height $H$ of the kernel;
~~~
  - while **kernel** is formally a 2D matrix (we say a convolution has
    _kernel size_ $H×W$), in deep learning, the whole
    four-dimensional weight tensor $⇶K$ can be also called a kernel;
~~~
- the number of output **channels** $O$
~~~
  - we also say that a convolution has $O$ **filters** (each filter
    computing one channel);
~~~
- the **stride**, which determines the interval at which the input is
  sampled—computing output for every **stride**-th pixel means that
  a stride of 2 halves the output's spatial dimensions.

~~~
Considering an input image with $C$ channels, the convolution layer is then
parametrized by a weight tensor $⇶K$ of total size $W × H × C × O$ and a bias $→b$
...
The 96 convolutional filters of size $11×11×3$ learned by the first
convolutional layer of AlexNet on the $224×224×3$ input images. The top 48
filters were learned on GPU 1 while the bottom 48 filters were learned on GPU 2.

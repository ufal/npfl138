...
For functions $x$ and $w$, the _convolution_ $w \ast x$ is defined as
$$(w \ast x)(t) = ∫x(t - a)w(a)\d a.$$
...
~~~
Closely related to convolution is _cross-correlation_, where the matrix $⇉K$
is flipped:
$$(⇉K \star ⇉I)_{i, j} = ∑\nolimits_{m, n} ⇉I_{i+m, j+n} ⇉K_{m, n}.$$
...
~~~
- the **stride**, which determines that every output pixel is computed by
  applying the filter every **stride** pixels in the input (e.g., the output
  is half the size if stride is 2).
...
~~~
Note that convolution is applied locally across the spatial dimensions (width and height),
whereas the input channels are combined in a fully connected manner.
...
~~~
Batch normalization $BN$ is added just before a nonlinearity $f$, and it is redundant
to add bias beforehand (because it would cancel out). Therefore, we replace
$f(⇉W→x + →b)$ by
$$f(\textit{BN}(⇉W→x)).$$
...
honour the properties of the feature maps, most notably the shift invariance.
We therefore normalize each channel across not only the minibatch, but also
across all corresponding spatial/temporal locations.

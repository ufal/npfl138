...
Assume that we have a sequence of $n$ words represented by a matrix $⇉X ∈ ℝ^{n×d}$.
...
where a language model predicts the following word based on the sentence prefix.
~~~
...
language models' hidden states is used.

![w=68%,h=center](elmo_embedding.png)

~~~
To be exact, the authors propose to take a (trainable) weighted combination of
the input embeddings and the outputs on the first and second LSTM layers.
...
The baseline BERT model consists of 12 Transformer layers:
...
The input to the BERT model is a sequence of subwords, namely their identifiers.
...
- trainable positional embeddings (not the sinusoidal embeddings, but I do not
  know why),
~~~
- trainable segment embeddings, which indicate whether a token belongs to sentence
...
are used, with a 30k WordPiece vocabulary.
...
For finetuning, a dropout rate of 0.1 is used, usually a very small number of epochs (2–4) suffice,

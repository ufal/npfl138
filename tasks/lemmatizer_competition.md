...
template, which among others generates test set annotations in the required format. As usual,
you can evaluate the predictions using the
[MorphoDataset](https://ufal.mff.cuni.cz/~straka/courses/npfl138/2526/docs/datasets/morpho_dataset/)
class, either by running `python3 -m npfl138.datasets.morpho_dataset --task=lemmatizer --evaluate=path --dataset=dev/test`
